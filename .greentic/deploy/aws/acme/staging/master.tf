...
[ {
  ...
  "logConfiguration": {
    "logDriver": "awslogs",
    "options": {
      "awslogs-group": "/greentic/acme/staging/greentic_acme_component",
      "awslogs-region": var.aws_region,
      "awslogs-stream-prefix": "greentic.acme.component"
    }
  }
} ]
EOF
...
resource "aws_cloudwatch_log_group" "runner_greentic_acme_component_logs" {
  name = "/greentic/acme/staging/greentic_acme_component"
  retention_in_days = var.log_retention_days
}

resource "aws_appautoscaling_target" "runner_greentic_acme_component_as" {
  max_capacity       = 2 + var.autoscaling_max_extra
  min_capacity       = 2
  resource_id        = "service/${aws_ecs_cluster.nats.name}/${aws_ecs_service.runner_greentic_acme_component_service.name}"
  scalable_dimension = "ecs:service:DesiredCount"
  service_namespace  = "ecs"
}

resource "aws_appautoscaling_policy" "runner_greentic_acme_component_cpu" {
  name               = "greentic.acme.component-cpu-policy"
  policy_type        = "TargetTrackingScaling"
  resource_id        = aws_appautoscaling_target.runner_greentic_acme_component_as.resource_id
  scalable_dimension = aws_appautoscaling_target.runner_greentic_acme_component_as.scalable_dimension
  service_namespace  = aws_appautoscaling_target.runner_greentic_acme_component_as.service_namespace
